...
Adapted from https://github.com/alexandrebarachant/muse-lsl/blob/master/examples/utils.py 
...
#Setting up a notch filter to remove AC current noise from signal
...
def compute_band_powers(eegdata, fs):
    ...
    meanLowBeta = np.mean(PSD[ind_low_beta, :], axis=0)
    # High Beta 20-35
    ind_high_beta, = np.where((f >= 20) & (f < 35))
    meanHighBeta = np.mean(PSD[ind_high_beta, :], axis=0)
    # Gamma 35-45
    ...
    feature_vector = np.concatenate((meanDelta, meanTheta, meanAlpha,
                                     meanBeta, meanLowBeta, meanHighBeta, meanGamma), axis=0)
