...
USE_LIBLO = False  # s.name != 'nt'
if USE_LIBLO:
    from liblo import ServerThread, Address, Message, Bundle, send, Server, make_method
    print('using Liblo')
else:
...
usage = "python museProc_tenere.py --museip 192.168.1.54 --museport 58775 --oscip 192.168.1.24 --oscport 7878 --sparseip 192.168.1.24 --sparseport 1234"
parser = ArgumentParser(usage=usage)
'''parser.add_argument("-m", "--museip",
                  dest="museip", type=str, default="192.168.1.45",
                  help="IP address of device muse direct OSC streaming")'''
parser.add_argument("-l", "--museport",
                  dest="museport", type=int, default=58775,
                  help="port to listen for muse data on")
parser.add_argument("-o", "--oscip",
                  dest="oscip", type=str, default="127.0.0.1",
                  help="IP address of localhost (Wavelength) to send OSC message to")
parser.add_argument("-p", "--oscport",
                  dest="oscport", type=int, default=5002,
                  help="The port that Wavelength is listening on")
parser.add_argument("-r", "--sparseip",
                  dest="sparseip", type=str, default="192.168.1.24",
                  help="IP address of the device to send OSC status message to")
parser.add_argument("-s", "--sparseport",
...
class FFTServer():
    ...
    def __init__(self, incoming, outgoing,  sparseOutput=None, config={}, device_source='muse',
                 software_source='musedirect', debug_outputs=True, verbose=True):
        ...
        if isinstance(self.incoming, str):  # LSL inlet
            ...
            if USE_LIBLO:
                #ServerThread.__init__(self, {'address':'museip','port':'port'})
                self._osc_server = ServerThread(incoming['port'])
                #self._osc_server = FFTServer({'address':'museip','port':'museport'})
 ...
 #                   raise NotImplementedError
                    self._client = udp_client.SimpleUDPClient(outgoing['address'], outgoing['port'])
    ...
    def _init_processing(self, config):
        ...
        if config['filter_bank']:
            ...
            for name, coeff in config['filter_bank'].items():
                ...
                self.filter_bank[name] = {'b': coeff[0],
                                          'a': coeff[1],
                                          'zi': zi}
            print("initialized filter bank")


        # Initialize processing parameters
        self.window_len = int(config['window_len'])
        self.step = int(config['step'])
        print("initialized processing parameters")
        ...
        self.psd_buffer = ut.CircularBuffer(psd_buffer_len, 129,
                                            self.n_channels)
        print("initialized processing buffers")
        ...
        self.ratio_powers=0
        self.ratio_names=0
        print("initialized scoring histograms (328)")
        ...
        if config['filter7']:
            ...
            print("initialized body filters (391)")

    #@make_method('/Fede/eeg', 'ffff')
...
#        eyeH = np.reshape([np.square(norm_diff_eyes)], (1, 1))
       ...
       # Send outputs at a reduced sampling rate
        if self.smooth_eeg_buffer.pts%3==0 :
            self._send_output_vec(smooth_eeg_samples, timestamp, '/Muse-EB72/eeg')
            if (self.blink > 0):
    ...
    def _process_acc(self, samples, timestamp):
        self._send_output_vec(samples,0,'/Muse-EB72/acc')
    ...
    def _process_gyro(self, samples, timestamp):
        self._send_output_vec(samples,0,'/Muse-EB72/gyro')
...
#                    send(out, Bundle(timestamp, message))
                    send(out, message)
            if self.verbose:
                print('Output vec: {}'.format(output))
...
#
                ...
                print("Started OSC server collection")
                #self._osc_server.add_method('/Fede/eeg', None, self._update_eeg_liblo_osc())#('/Fede/eeg', [0,0,0,0]))
         #       self.make_method('/Fede/muse/eeg', None, self._update_eeg_liblo_osc)#(arguments.port, None))#('/Fede/muse/eeg', arguments[1]))
          #      self.make_method('/Fede/muse/acc', None, self._update_acc_liblo_osc)#(arguments.port, None))
           #     self.make_method('/Fede/muse/gyro', None, self._update_gyro_liblo_osc)#(arguments.port, None))

               # self._osc_server.add_method('/Fede/muse/eeg', None, self._update_eeg_liblo_osc)#(arguments.port, None))#('/Fede/muse/eeg', arguments[1]))
                #self._osc_server.add_method('/Fede/muse/acc', None, self._update_acc_liblo_osc)#(arguments.port, None))
                #self._osc_server.add_method('/Fede/muse/gyro', None, self._update_gyro_liblo_osc)#(arguments.port, None))
                #self._osc_server.register_methods()
               
                #print(arguments)
            else:
                self._dispatcher.map("/Muse-EB72/eeg",print)

                self._dispatcher.map("/Muse-EB72/eeg", self._update_eeg_python_osc, "EEG")

                self._osc_server = osc_server.ThreadingOSCUDPServer(("192.168.1.54", self.incoming["port"]), self._dispatcher)
                print('OSC server initialized at port {}.'.format(self.incoming["port"]))
                self._server_thread = Thread(target=self._osc_server.serve_forever)
                print("Thread set up")
...
if __name__ == '__main__':
    ...
    config = {'fs': FS,
              ...
              'filter_bank': {'delta': (b_delta, a_delta), 'theta': (b_theta, a_theta), 'alpha': (b_alpha, a_alpha), 'beta': (b_beta, a_beta)},
    ...
    fft_server = FFTServer(({'port':arguments.museport}),  # 'MEG', #  #
                            ({'address': arguments.oscip, 'port': arguments.oscport}),
                            ({'address': arguments.sparseip, 'port': arguments.sparseport}),
                           config=config,
                           device_source='muse',  #vive,  leroy
                           software_source='musedirect',
                           debug_outputs=True,
                           verbose=True)
    fft_server.start()
 ...
 #       try:
       ...
       # print('Output: {}'.format(fft_server._update_eeg_python_osc))
        time.sleep(1)
'''
        except:
            fft_server.stop()
